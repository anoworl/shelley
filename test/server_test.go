...
func TestServerEndToEnd(t *testing.T) {
	...
	t.Run("CreateAndListConversations", func(t *testing.T) {
		...
		conv, err := database.CreateConversation(context.Background(), &slug, true, nil, nil, nil)
		if err != nil {
...
func TestConversationCleanup(t *testing.T) {
	...
	// Create a conversation
	// Using database directly instead of service
	conv, err := database.CreateConversation(context.Background(), nil, true, nil, nil, nil)
	if err != nil {
		t.Fatalf("Failed to create conversation: %v", err)
	}
...
func TestSlugEndToEnd(t *testing.T) {
	...
	ctx := context.Background()
	testTitle := "テスト会話タイトル"
	conv, err := database.CreateConversation(ctx, &testTitle, true, nil, nil, nil)
	if err != nil {
		t.Fatalf("Failed to create conversation: %v", err)
	}
